 ...
 * - PRO (1) - $15/month, includes PRO features
 * - BUSINESS (2) - $50/month, includes PRO + BUSINESS features
 *
 * **Plan Hierarchy Logic:**
 * - PRO users can access PRO features (hierarchy: PRO >= PRO)
 * - BUSINESS users can access both PRO and BUSINESS features (hierarchy: BUSINESS >= PRO/BUSINESS)
 * - FREE users cannot access paid features
 *
 * @ai-prompt When modifying this middleware:
 * - ALWAYS respect plan hierarchy (BUSINESS >= PRO >= FREE)
 * - hasFeatureAccess() uses cached data (5min TTL, see stripe.service.ts)
 * - requireSubscription() checks specific plan level
 * - requireActiveSubscription() checks ANY paid plan (PRO or BUSINESS)
 ...
 *
 * @param requiredPlan - Minimum required plan (PlanType.PRO or PlanType.BUSINESS)
 * @returns Fastify middleware function
 *
 * @example
 * ```typescript
 * // PRO feature (accessible to PRO and BUSINESS users)
 ...
 *   preHandler: [authMiddleware, requireSubscription(PlanType.BUSINESS)]
 * }, handler)
 *
 * // Any paid plan (PRO or BUSINESS)
