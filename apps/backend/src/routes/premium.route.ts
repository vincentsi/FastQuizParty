...
export async function premiumRoutes(fastify: FastifyInstance) {
  // ===== PRO Features (accessible to PRO and BUSINESS) =====
  fastify.register(async function (fastify) {
     ...
     *
     * Accessible to users with PRO or BUSINESS plan
     */
  ...
  // ===== BUSINESS Features (accessible to BUSINESS only) =====
  fastify.register(async function (fastify) {
    fastify.addHook('preHandler', authMiddleware)
    fastify.addHook('preHandler', requireSubscription(PlanType.BUSINESS))

    /**
     * Example BUSINESS feature
     * GET /api/premium/business-feature
     *
     * Accessible only to users with BUSINESS plan
     */
