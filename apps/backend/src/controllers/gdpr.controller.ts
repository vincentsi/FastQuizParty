...
import { logSecurityEvent, logUserAction } from '@/utils/logger'
...
export class GDPRController {
  ...
  exportUserData = asyncHandler(
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      // Force explicit JSON serialization to ensure data is not lost
      // Fastify's automatic serialization can cause issues with objects containing null values
      // We manually stringify to guarantee proper serialization
      const response = SuccessResponse.ok(userData)
      const jsonResponse = JSON.stringify(response)
      return reply.status(200).type('application/json').send(jsonResponse)
    }
  )
  ...
  deleteUserData = asyncHandler(
    async (
      ...
      logSecurityEvent(request, 'gdpr_data_deletion', 'critical', {
        userId,
        reason,
      })
  ...
  anonymizeUserData = asyncHandler(
    async (
      ...
      if (confirmEmail !== userEmail) {
        return reply
          .status(400)
          .send(ErrorResponse.badRequest('Email confirmation does not match'))
      }
